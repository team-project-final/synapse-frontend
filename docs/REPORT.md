...
### 배경 / 이전 상태
- 2단계 에디터에서 제목 칸을 추가했으나 **미리보기에 제목이 안 나오고**(본문만 렌더), 제목 입력 시 미리보기가 갱신되지 않았다(리스너 없음).
- 헤딩(H1/H2) 툴바 버튼이 `# `를 **커서 위치**에 삽입해 줄 중간에서 누르면 헤딩으로 렌더되지 않았다(마크다운 헤딩은 줄 시작 필요).

### 변경 사항 (`note_editor_screen.dart`)
| 항목 | 내용 |
|------|------|
| 미리보기 제목 | `_buildPreview()` 신설 — 미리보기 최상단에 제목(headlineSmall) + 본문(MarkdownBody). 데스크탑/모바일 공용 |
| 제목 실시간 갱신 | `_titleController` 리스너 추가 → 제목 타이핑 시 미리보기 갱신 |
| 헤딩 버그픽스 | `_insertLinePrefix()` 신설 — `# `/`## `를 **현재 줄 맨 앞**에 삽입(커서 위치 아님). H1/H2 버튼이 이를 사용 |

### 검증
- `flutter analyze` 0 경고 / 위젯 테스트 12/12 통과.
- 로컬 실데이터(knowledge-svc 8082 직결)로 제목 미리보기·헤딩 육안 확인 완료.
