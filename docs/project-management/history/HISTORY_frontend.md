...
| (진행) | 2026-06-10 | admin 시스템 설정 — 플랜 쿼터(조회)·피처 플래그·레이트리밋 실연동 | platform PLAT-072 |

**의도적 보류/잔여**: 로그인 바이패스 유지(마무리 시 실 로그인 전환) · 결제/멤버 페이지네이션 없음(첫 N건) · 아바타 업로드/GDPR(백엔드 미구현) · 신고/그룹/게임화/콘텐츠(타 서비스 소관)
