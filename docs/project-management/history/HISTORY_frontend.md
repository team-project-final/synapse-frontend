...
| (진행) | 2026-06-10 | 설정 허브 알림 mock 사본 제거(실연동 화면 재사용) + admin 셸 로그아웃 연결·가짜 환경 드롭다운 제거 | — |

**의도적 보류/잔여**: 로그인 바이패스 유지(마무리 시 실 로그인 전환) · 결제/멤버 페이지네이션 없음(첫 N건) · admin 시스템설정/GDPR(백엔드 미구현) · 신고/그룹/게임화/콘텐츠(타 서비스 소관)
