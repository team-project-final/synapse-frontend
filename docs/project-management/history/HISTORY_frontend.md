...
| (진행) | 2026-06-10 | 알림 마무리(이슈 #20) — 미읽음 뱃지 30초 폴링·분류별 클릭 라우팅·'더 보기' 페이지네이션 | platform #79 |

**의도적 보류/잔여**: 로그인 바이패스 유지(마무리 시 실 로그인 전환) · 결제/멤버 페이지네이션 없음(첫 N건) · admin 시스템설정/GDPR(백엔드 미구현) · 신고/그룹/게임화/콘텐츠(타 서비스 소관) · settings 허브 알림 설정 mock 사본 정리 필요
