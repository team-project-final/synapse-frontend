...
### 1.2 요구사항 분석
- [x] 알림 목록 UI 요건 분석 (무한 스크롤은 범위 제외 — 첫 N건, 알려진 한계로 기록)
...
- [ ] 알림 타입별 라우팅 설계 (알림 클릭 → 관련 페이지 이동) — 미구현
- [ ] Duration(final) 갱신
...
### 1.8 Service + Test
- [x] 알림 목록 로드·읽음 처리 구현 (무한 스크롤은 범위 제외)
- [ ] 안읽은 알림 카운트 실시간 갱신 (폴링 30초 간격) — 미구현 (진입 시 로드)
- [x] 알림 설정 관리 로직 (토글 on/off → API 호출)
- [x] Unit 테스트 (알림 상태 전이, 읽음 처리)

### 1.9 Controller + Test
- [x] 안읽은 카운트 뱃지 구현 (사이드바 unread-count 뱃지)
- [x] 알림 목록 페이지 Widget 구현 (ListView — 무한 스크롤 제외)
- [x] 알림 항목 Widget 구현 (아이콘 + 제목 + 시간 + 읽음/안읽음 스타일)
- [ ] 알림 클릭 시 상세 이동 로직 (타입별 라우팅) — 미구현
...
**Step 8 Status**: [x] Done (PR #37, 2026-06 — 인박스·읽음/모두읽음·unread 뱃지·설정·카테고리 탭 필터 실연동. 잔여: 무한 스크롤(첫 N건)·30초 폴링·타입별 클릭 라우팅은 범위 제외. ⚠ settings 허브의 notification_settings_screen.dart는 미연동 mock 사본 — 정리 필요)
...
**Step 9 Status**: [x] In Progress (platform 몫인 admin 접근 제어는 PR #34로 완료 — JWT roles 기반 라우트/메뉴 가드. 신고 목록·처리 연동은 engagement-svc 소관이며, 백엔드 ADMIN role 발급 메커니즘(F8, platform-svc 이슈 #86) 합의 대기로 차단 상태)
